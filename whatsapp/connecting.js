 ...
 */
    ...
    */
    ...
    async sendFile(jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) {
      ...
      if (!type) if (options.asDocument) options.asDocument = false
      let mtype = ''
    ...
    fakeStatus(jid, teks, capt, thumb) {
    return this.sendMessage(jid, teks, MessageType.text, {
        quoted: {
          key: {
            fromMe: false,
    ...
    fakeLink(jid, teks, thumb, title, url, quoted ){
			this.sendMessage(jid, teks, MessageType.text, { thumbnail: thumb, sendEphemeral: true, quoted: quoted, contextInfo: { externalAdReply : { 
			  title: title, 
			  previewType: "PHOTO",
			  thumbnail : thumb,
			  sourceUrl : url
